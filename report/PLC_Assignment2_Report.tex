...
\begin{titlepage}
    ...
    {\Huge\bfseries PLC Assignment 2\par}
    \vspace{0.7cm}
    {\Large Programming Language and Compiler Group Project Report\par}
    ...
    {\Large
    Kyal Sin Lin Lett\par
    Wai Yan Aung\par
    Aakriti Poudel\par}

    \vspace{1.9cm}
    {\Large\bfseries Course: Programming Language and Compiler\par}

    \vspace{1.8cm}
    {\Large\bfseries Submitted to\par}
    \vspace{0.3cm}
    {\Large Prof. Phan Min Dung\par}
    {\Large Mr. Akraradet Sinsamersuk\par}
...
The goal of this project was to build a compact yet meaningful language that demonstrates core compiler concepts while still remaining understandable and testable within one course assignment. The project requirements asked for support for four basic types, static typing, arithmetic precedence, boolean expressions, assignment statements, \texttt{if-then-else}, \texttt{while}, function abstraction with value parameter passing, and a \texttt{print()} function. Our implementation satisfies these requirements and further packages them into a small IDE-style graphical application.
